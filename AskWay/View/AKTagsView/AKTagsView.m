...
//  AskWay
//
//  Created by damion on 2017/12/4.
...
@property (nonatomic) AKTagModel *tagModel;
...
@interface AKEqualSpaceFlowLayout : UICollectionViewFlowLayout
...
@implementation AKEqualSpaceFlowLayout
...
@interface AKTagsView ()<UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>

@property (strong, nonatomic) UICollectionView *collectionView;
@property (strong, nonatomic) NSMutableArray<NSString *> *tagsMutableArray;
@property (strong, nonatomic) NSMutableArray<AKTagModel *> *tagModels;

@end


@implementation AKTagsView
...
- (void)setTagsArray:(NSArray<NSString *> *)tagsArray {
    ...
    [tagsArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        AKTagModel *tagModel = [[AKTagModel alloc] initWithName:obj font:self.tagFont];
...
- (void)deSelectTagAtIndex:(NSUInteger)index animate:(BOOL)animate {
    ...
    AKTagModel *tagModel = self.tagModels[index];
...
- (void)deSelectAll {
    for (NSIndexPath *indexPath in self.collectionView.indexPathsForSelectedItems) {
        AKTagModel *tagModel = self.tagModels[indexPath.row];
        tagModel.selected = NO;
...
- (AKTagCell *)cellForItemAtIndex:(NSInteger)index {
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:index inSection:0];
    return (AKTagCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
}
...
- (void)addTag:(NSString *)tagName {
    ...
    AKTagModel *tagModel = [[AKTagModel alloc] initWithName:tagName font:self.tagFont];
    [self.tagModels addObject:tagModel];
    [self.collectionView reloadData];
    [self invalidateIntrinsicContentSize];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    AKTagCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kTagCellID forIndexPath:indexPath];
...
- (void)setCell:(AKTagCell *)cell selected:(BOOL)selected {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    AKTagCell *tagCell = (AKTagCell *)cell;
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        AKEqualSpaceFlowLayout *flowLayout = [[AKEqualSpaceFlowLayout alloc] init];
        flowLayout.delegate = self;
        _collectionView = [[UICollectionView alloc] initWithFrame:self.bounds collectionViewLayout:flowLayout];
        _collectionView.backgroundColor = [UIColor clearColor];
        [_collectionView registerClass:[AKTagCell class] forCellWithReuseIdentifier:kTagCellID];
...
- (NSMutableArray<AKTagModel *> *)tagModels {
    if (!_tagModels) {
