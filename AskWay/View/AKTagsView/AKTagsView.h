...
@protocol AKTagsViewDelegate <NSObject>

@optional
- (BOOL)tagsView:(AKTagsView *)tagsView shouldSelectTagAtIndex:(NSUInteger)index;
- (void)tagsView:(AKTagsView *)tagsView didSelectTagAtIndex:(NSUInteger)index;

- (BOOL)tagsView:(AKTagsView *)tagsView shouldDeselectItemAtIndex:(NSUInteger)index;
- (void)tagsView:(AKTagsView *)tagsView didDeSelectTagAtIndex:(NSUInteger)index;

//超过最多选中个数，可在这个方法中做自定义提示，如提示用户超过最多选中项
- (void)tagsViewDidBeyondMaximumNumberOfSelection:(AKTagsView *)tagsView;

//允许对Cell进行自定义操作
- (void)tagsView:(AKTagsView *)tagsView willDispayCell:(AKTagCell *)tagCell atIndex:(NSUInteger)index;
