...
@implementation NSString (TypeCheck)

+ (BOOL)isBlankString:(NSString *)string {
    if (string == nil || [string isEqual:[NSNull null]] || ![string isKindOfClass:[NSString class]] || string.length == 0 || [string isEqualToString:@""]) {
        return YES;
    }
    ...
    switch (length) {
        ...
        case 18:
            ...
            if(numberofMatch >0) {
                ...
                //                if ([M isEqualToString:[value substringWithRange:NSMakeRange(17,1)]])//区分大小写
...
+ (BOOL)isPhoneNumber:(NSString*)phone {
     ...
     *
     */
    //NSString *regex = @"(^(\\d{3,4}-)?\\d{7,8})$|(1[34578]{1}[0-9]{9})";
    NSString *regex = @"[-+*#,;\\d]*";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
